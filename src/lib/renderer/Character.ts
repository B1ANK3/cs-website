...
export class Character {
  ...
  private onIdleUpdate(dt: number): void {
    if (this.randomWalkBehavior.shouldPickNewTarget()) {
      ...
      const randomNode = this.pathFinding.getRandomConnectedNode(
        this.entity.pathData.nodes[this.entity.pathData.currentNodeIndex]?.id ||
          'entry-left'
      );
  ...
  private onWalkingUpdate(dt: number): void {
    ...
    if (this.entity.pathData.nodes.length > 0) {
      ...
      if (
        ...
        this.entity.pathData.nodes.length - 1
      ) {
  ...
  public pathTo(targetNodeId: string | number): void {
    const currentNode = this.entity.pathData.nodes[this.entity.pathData.currentNodeIndex];
    const currentId = currentNode?.id ?? 'entry-left';
  ...
  private moveAlongPath(distance: number): void {
    ...
    while (remaining > 0 && this.entity.pathData.currentNodeIndex < nodes.length - 1) {
      ...
      const moveDistance = Math.min(remaining, segmentDistance * (1 - this.entity.pathData.progress));
