...
export const DEFAULT_BEHAVIOR_CONFIG: BehaviorConfig = {
    ...
    turnAndTalkChance: 0.15, // 15% chance to turn and talk during idle
...
/**
 * Turn and talk behavior generator
 * Determines when a character should stop and talk
 */
export class TurnAndTalkBehavior {
    private turnAndTalkChance: number;
    private lastTalkTime: number = 0;
    private minTimeBetweenTalk: number = 8000; // Minimum 8 seconds between talk attempts

    constructor(chance: number = DEFAULT_BEHAVIOR_CONFIG.turnAndTalkChance) {
        this.turnAndTalkChance = chance;
        this.lastTalkTime = Date.now();
    }

    /**
     * Check if character should turn and talk
     * Only checks once every minTimeBetweenTalk milliseconds to avoid too frequent transitions
     */
    public shouldTurnAndTalk(): boolean {
        const elapsed = Date.now() - this.lastTalkTime;
        if (elapsed < this.minTimeBetweenTalk) {
            return false;
        }

        if (shouldCharacterTalk(this.turnAndTalkChance)) {
            this.lastTalkTime = Date.now();
            return true;
        }
    ...
    public setMinTimeBetweenTalk(ms: number): void {
        this.minTimeBetweenTalk = ms;
    }
}
